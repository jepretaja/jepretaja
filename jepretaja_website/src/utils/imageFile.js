 ...
 *
 * @param {{sisiMaks?: number, targetByte?: number, rasio?: number|null}} opsi
 *   `rasio` = lebar/tinggi tujuan (default 16:9); `null` mempertahankan rasio asli.
...
export async function kompresGambar(file, { sisiMaks = 1920, targetByte = 400 * 1024, rasio = 16 / 9 } = {}) {
  ...
  // `rasio: null` = jangan dipotong sama sekali. Unggahan Explore memakai ini
  // supaya foto potret tidak dipaksa menjadi 16:9 dan kehilangan kepala/kakinya.
  const rasioTarget = rasio == null ? rasioAsli : rasio;
  const lebarCrop = rasioAsli > rasioTarget ? Math.round(tinggiAsli * rasioTarget) : lebarAsli;
  const tinggiCrop = rasioAsli > rasioTarget ? tinggiAsli : Math.round(lebarAsli / rasioTarget);
