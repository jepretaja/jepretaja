...
import { processWithdrawal, markWithdrawalManual, disburseWithdrawalIris } from '../../utils/adminActions';
import StatusBadge from '../../components/StatusBadge';
import ErrorState from '../../components/ErrorState';
import { formatCurrency, formatDateTime } from '../../utils/format';

/** Sinkron dengan IRIS_BANK_CODE di api/_lib/irisClient.js — hanya bank yang
 *  kodenya sudah terkonfirmasi ke Midtrans Iris yang boleh ditawarkan lewat
 *  tombol otomatis; bank lain tetap lewat transfer manual di bawahnya. */
const BANK_DIDUKUNG_IRIS = ['BCA', 'BNI', 'BRI', 'Mandiri', 'CIMB Niaga', 'Permata', 'Danamon', 'BTN'];
...
export default function WithdrawalDetail() {
  ...
  const handleIris = async () => {
    const ok = await confirm({
      title: 'Kirim ke Midtrans Iris?',
      message: `Payout ${formatCurrency(withdrawal.amount)} akan diajukan ke Midtrans Iris. Dana baru benar-benar terkirim setelah disetujui approver di Iris Dashboard — status di sini akan otomatis berubah lewat webhook begitu itu terjadi.`,
      confirmLabel: 'Ya, Kirim ke Iris',
    });
    if (!ok) return;
    setProcessing(true);
    setError(null);
    try {
      await disburseWithdrawalIris({ withdrawalId: id });
    } catch (err) {
      setError(err.message || 'Gagal mengirim payout ke Midtrans Iris.');
    } finally {
      setProcessing(false);
    }
  };
    ...
    <div>
      ...
      <div className="grid grid-2">
        ...
        <div className="card">
          ...
          {isProcessing && (
            <div>
              {withdrawal.irisReferenceNo ? (
                <p className="text-meta mb-sm">
                  Sudah dikirim ke Midtrans Iris (ref: {withdrawal.irisReferenceNo}, status terakhir: {withdrawal.irisStatus || 'queued'}).
                  Menunggu approval di Iris Dashboard — status di sini akan berubah otomatis begitu Midtrans mengonfirmasi.
                  Kalau macet terlalu lama, tetap bisa ditandai manual di bawah.
                </p>
              ) : BANK_DIDUKUNG_IRIS.includes(withdrawal.bankCode) ? (
                <>
                  <p className="text-meta mb-sm">
                    Bank tujuan ({withdrawal.bankCode}) didukung disbursement otomatis Midtrans Iris.
                  </p>
                  <button className="btn btn-primary btn-block mb-md" disabled={!canProcess || processing} onClick={handleIris}>
                    {processing ? 'Mengirim...' : 'Cairkan Otomatis via Midtrans Iris'}
                  </button>
                </>
              ) : (
                <p className="text-meta mb-sm">
                  Bank tujuan ({withdrawal.bankCode || '-'}) belum didukung disbursement otomatis. Transfer manual lewat internet banking lalu tandai di sini.
                </p>
              )}
