...
import { readBody, requireString, requireOneOf, notFound, conflict, HttpError } from '../http.js';
...
export async function releaseEscrow(req) {
  ...
  return db.runTransaction(async (tx) => {
    ...
    const saldoSebelum = Number(wallet.availableBalance) || 0;
    const totalSebelum = Number(wallet.totalEarnings) || 0;
    const nominal = Number(escrow.creatorShare) || 0;
    const saldoSesudah = saldoSebelum + nominal;

    // Dana ini sebelumnya masuk pendingBalance saat confirmManualPayment/
    // webhook Midtrans (escrow_hold). Sekarang dilepas ke availableBalance,
    // jadi pendingBalance HARUS dikurangi nominal yang sama — kalau tidak,
    // dana yang sama kelihatan dobel: sudah bisa ditarik (availableBalance)
    // tapi masih tercatat "tertahan" (pendingBalance) selamanya.
    const pendingSebelum = Number(wallet.pendingBalance) || 0;
    const pendingSesudah = Math.max(0, pendingSebelum - nominal);

    tx.set(walletRef, {
      creatorId: escrow.creatorId,
      pendingBalance: pendingSesudah,
...
export async function processRefund(req) {
  ...
  // Escrow dibaca DI LUAR transaksi berikut sekadar untuk tahu apakah ada
  // dana yang perlu dibersihkan — dibaca ulang di dalam transaksi supaya
  // tetap konsisten kalau berubah di antara waktu ini.
  const escrowRef = db.collection('escrow_transactions').doc(refund.bookingId);

  await db.runTransaction(async (tx) => {
    const current = await tx.get(refundRef);
    if (!current.exists || current.data().status !== 'pending') throw conflict('Refund sudah diproses admin lain.');

    // Dana yang sedang direfund ini sebelumnya ditahan lewat escrow (masuk
    // wallets.pendingBalance saat pembayaran diverifikasi). Refund biasa
    // (bukan keputusan dispute) sebelumnya HANYA mengubah status booking/
    // payment/refund, dan tidak pernah membersihkan escrow_transactions
    // maupun pendingBalance — akibatnya HeldFunds.jsx terus menampilkan
    // dana ini sebagai "masih tertahan" dan saldo pending creator tidak
    // pernah berkurang, padahal uangnya sudah balik ke customer.
    const eSnap = await tx.get(escrowRef);
    const escrow = eSnap.exists ? eSnap.data() : null;
    const perluBersihkanEscrow = escrow && escrow.releaseStatus === 'pending';

    if (perluBersihkanEscrow) {
      const walletRef = db.collection('wallets').doc(escrow.creatorId);
      const wSnap = await tx.get(walletRef);
      const wallet = wSnap.data() || {};
      const pendingBefore = Number(wallet.pendingBalance) || 0;
      const nominal = Number(escrow.creatorShare) || 0;
      const pendingAfter = Math.max(0, pendingBefore - nominal);
      ...
      tx.set(db.collection('wallet_transactions').doc(), {
        ...
        balanceAfter: pendingAfter,
        status: 'success',
        note: 'Dana pending dibatalkan karena refund ke customer disetujui.',
        createdAt: FieldValue.serverTimestamp(),
      });
      ...
      tx.update(escrowRef, {
        ...
        refundedAmount: Number(escrow.amount) || 0,
        releasedAt: FieldValue.serverTimestamp(),
        releasedBy: actor.uid,
      });
    }
