...
export async function createBooking(req) {
  ...
  const voucherCode = hasil.breakdown.voucherCode;
  const voucherRef = voucherCode ? db.collection('promotions').doc(voucherCode) : null;
  ...
  await db.runTransaction(async (tx) => {
    ...
    // Kuota voucher divalidasi ULANG di sini (bukan hanya di hitungHarga di
    // luar transaksi) dan usedCount-nya dinaikkan sekarang juga.
    // hitungHarga membaca promotions di luar transaksi hanya untuk hitung
    // potongan harga; tanpa pengecekan+kenaikan di dalam transaksi ini,
    // usedCount tidak pernah bertambah sama sekali (voucher berkuota bisa
    // dipakai tanpa batas), dan dua booking bersamaan dengan voucher yang
    // sama juga bisa sama-sama lolos melewati kuota yang tersisa.
    if (voucherRef) {
      const vSnap = await tx.get(voucherRef);
      if (!vSnap.exists || vSnap.data().active === false) {
        throw conflict('Voucher sudah tidak berlaku, silakan ulangi tanpa voucher ini.');
      }
      const v = vSnap.data();
      if (typeof v.quota === 'number' && (v.usedCount || 0) >= v.quota) {
        throw conflict('Kuota voucher baru saja habis, silakan ulangi tanpa voucher ini.');
      }
