...
/**
 * Batas maksimum yang boleh menjadi hak creator dari dana yang tertahan.
 *
 * `payoutAmount` di atas mengembalikan nominal KOTOR (yang dibayar customer,
 * termasuk komisi platform) — dipakai untuk memvalidasi/mengukur refund
 * karena refund diukur dari uang yang customer bayarkan. Tapi hak creator
 * hanya sebesar bagian BERSIH-nya (creatorShare), sama seperti jalur rilis
 * normal (releaseEscrow). Sebelumnya fungsi ini tidak ada, sehingga
 * keputusan "release_full" pada dispute memberi creator seluruh nominal
 * kotor — termasuk komisi platform yang seharusnya tidak pernah sampai ke
 * creator.
 */
export function creatorShareCap(escrow, grossHeld) {
  if (escrow && Number.isFinite(Number(escrow.creatorShare))) {
    return Math.max(0, Number(escrow.creatorShare));
  }
  // Booking lama tanpa dokumen escrow/creatorShare: tidak ada dasar untuk
  // memotong komisi, jadi perlakukan seperti sebelumnya (seluruh nominal).
  return grossHeld;
}
...
export async function resolveDispute(req) {
  ...
  return db.runTransaction(async (tx) => {
    ...
    // `held` = nominal KOTOR yang pernah dibayar customer — dipakai untuk
    // memvalidasi dan mengukur berapa yang bisa dikembalikan ke customer.
    const held = payoutAmount(booking, escrow);
    if (held <= 0) throw badRequest('Nominal dana yang ditahan untuk booking ini tidak valid.');

    // `shareCap` = nominal BERSIH (setelah komisi platform) — batas maksimum
    // hak creator, dan juga nominal yang sebenarnya tercatat di
    // wallet.pendingBalance sejak dana ditahan (lihat confirmManualPayment /
    // webhook Midtrans, yang menaikkan pendingBalance sebesar creatorShare,
    // BUKAN sebesar nominal kotor).
    const shareCap = creatorShareCap(escrow, held);
    ...
    // Memakai shareCap (bersih) sebagai basis, bukan held (kotor) — supaya
    // creator tidak pernah menerima lebih dari haknya, dan supaya
    // pendingBalance yang dikurangi cocok dengan nominal yang benar-benar
    // pernah ditambahkan ke sana. calculateDisputeAmounts sendiri sudah
    // membatasi toCreator ke >= 0 lewat Math.max, jadi kalau toCustomer
    // melebihi shareCap, toCreator otomatis jadi 0 — refund di atas hak
    // creator akan mengurangi komisi platform (lihat platformPortion di
    // bawah), bukan membuat creator berutang.
    const { toCreator, pendingAfter, creatorFromPending, customerFromPending } =
      calculateDisputeAmounts(shareCap, pendingBefore, toCustomer);
    const availableAfter = availableBefore + creatorFromPending;

    // Sisa dari nominal kotor setelah dikurangi bagian customer dan bagian
    // creator adalah komisi platform yang berhasil dipertahankan untuk
    // booking ini — bisa lebih kecil dari platformFee awal (bahkan 0) kalau
    // refund yang diputuskan melebihi shareCap. Tanpa ini, komisi platform
    // untuk booking yang pernah disengketakan tidak pernah tercatat di kas
    // platform sama sekali.
    const platformPortion = Math.max(0, held - toCustomer - toCreator);
    ...
    // Sama seperti releaseEscrow di jalur normal: komisi platform baru masuk
    // kas pada saat dana benar-benar dilepas, bukan saat pembayaran masuk.
    if (platformPortion > 0) {
      tambahKomisiPlatform(db, tx, { platformFee: platformPortion, bookingId });
    }
    ...
    writeAudit(db, actor, {
      ...
      targetId: disputeId,
      reason: note,
        metadata: { bookingId, creatorId, held, shareCap, toCreator, toCustomer, platformPortion, creatorFromPending, customerFromPending, pendingBefore, pendingAfter, availableBefore, availableAfter },
    }, tx);
