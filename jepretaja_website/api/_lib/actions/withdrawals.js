...
import { createIrisPayout, IRIS_BANK_CODE } from '../irisClient.js';
...
/**
 * Mengajukan disbursement otomatis lewat Midtrans Iris untuk withdrawal yang
 * sudah di-approve (status 'processing'). BUKAN pengganti markWithdrawalManual
 * — kalau bank tujuan belum terpetakan di IRIS_BANK_CODE, atau
 * MIDTRANS_IRIS_API_KEY belum diisi, admin tetap harus pakai jalur manual
 * yang sudah ada (lihat catatan di irisClient.js soal kenapa ini tidak bisa
 * dipaksa otomatis 100%).
 *
 * Withdrawal TIDAK langsung ditandai 'success' di sini — statusnya baru
 * final setelah approver Iris menyetujui dan webhook di
 * api/midtrans-iris-callback.js diterima. Di sini withdrawal hanya dicatat
 * "sedang dikirim ke Iris" (irisStatus: 'submitted') supaya panel tahu
 * jangan proses dobel.
 */
export async function disburseWithdrawalIris(req) {
  const actor = await requireAdmin(req, 'manage_withdrawal');
  const body = readBody(req);
  const withdrawalId = requireString(body.withdrawalId, 'withdrawalId');
  ...
  if (w.status !== 'processing') {
    throw conflict(`Hanya withdrawal berstatus "processing" yang bisa dikirim ke Iris (status sekarang: "${w.status}").`);
  }
  if (w.irisReferenceNo) {
    throw conflict('Withdrawal ini sudah pernah dikirim ke Iris sebelumnya.');
  }

  const bankCode = IRIS_BANK_CODE[w.bankCode];
  if (!bankCode) {
    throw badRequest(`Bank "${w.bankCode}" belum dipetakan untuk disbursement otomatis. Gunakan transfer manual untuk withdrawal ini.`);
  }

  let payout;
  try {
    payout = await createIrisPayout({
      beneficiaryName: w.bankAccountName,
      beneficiaryAccount: w.bankAccountNumber,
      beneficiaryBank: bankCode,
      amount: Number(w.amount) || 0,
      notes: `Withdrawal ${withdrawalId} — JepretAja`,
    });
  } catch (err) {
    if (err.code === 'iris-not-configured') {
      throw conflict('Midtrans Iris belum dikonfigurasi di server (MIDTRANS_IRIS_API_KEY kosong). Gunakan transfer manual.');
    }
    throw conflict(err.message || 'Gagal mengirim payout ke Midtrans Iris.');
  }

  await wRef.update({
    irisReferenceNo: payout.referenceNo,
    irisStatus: payout.status,
    irisSubmittedBy: actor.uid,
    irisSubmittedAt: FieldValue.serverTimestamp(),
  });
  await writeAudit(db, actor, {
    action: 'withdrawal_iris_submitted',
    targetType: 'withdrawal',
    targetId: withdrawalId,
    reason: null,
    metadata: { amount: w.amount, creatorId: w.creatorId, irisReferenceNo: payout.referenceNo },
  });

  return { status: 'submitted', irisReferenceNo: payout.referenceNo, irisStatus: payout.status };
}
