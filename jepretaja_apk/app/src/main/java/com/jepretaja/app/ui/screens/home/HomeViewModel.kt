...
class HomeViewModel @Inject constructor(
    ...
    val nearbyCreators: StateFlow<SeksiHome<List<CreatorModel>>> = pemicuMuatUlang
        .flatMapLatest { creatorRepository.streamNearby().map { it.take(12) } }
        .jadiSeksi()
    ...
    val popularCreators: StateFlow<SeksiHome<List<CreatorModel>>> = pemicuMuatUlang
        .flatMapLatest { creatorRepository.streamPopular(limit = 12) }
        .jadiSeksi()
        .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), SeksiHome.Memuat)
     ...
     * tertinggi".
     */
    val recommendedCreators: StateFlow<SeksiHome<List<CreatorModel>>> = popularCreators
        .map { seksi ->
            when (seksi) {
                SeksiHome.Memuat -> SeksiHome.Memuat
                is SeksiHome.Gagal -> seksi
                is SeksiHome.Isi -> SeksiHome.Isi(
                    seksi.data.sortedByDescending { skorRekomendasi(it) }.take(12),
                )
            }
        }
        .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), SeksiHome.Memuat)
    ...
    val latestWorks: StateFlow<SeksiHome<List<ExplorePostModel>>> = pemicuMuatUlang
        .flatMapLatest { exploreRepository.streamFeed(limit = 10) }
        .map { it.take(10) }
        .jadiSeksi()
        .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), SeksiHome.Memuat)
