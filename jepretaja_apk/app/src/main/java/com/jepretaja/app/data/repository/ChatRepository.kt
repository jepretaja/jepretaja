...
class ChatRepository @Inject constructor(private val db: FirebaseFirestore) {
    ...
    private val displayNameCache = mutableMapOf<String, Pair<String, String?>>()
    ...
    fun streamChats(userId: String): Flow<List<ChatModel>> = callbackFlow {
        ...
        fun listen(field: String) = db.collection(FirestorePaths.CHATS)
            ...
            .addSnapshotListener { snapshot, error ->
                ...
                backgroundScope.launch {
                    snapshot?.documents.orEmpty().forEach { document ->
                        val chat = document.toObject(ChatModel::class.java) ?: return@forEach
                        val otherId = if (chat.customerId == userId) chat.creatorId else chat.customerId
                        // CATATAN PERBAIKAN: guard lama ("otherPartyName sudah
                        // terisi dan bukan otherId") membuat blok ini SELALU
                        // dilewati — nama selalu langsung terisi saat chat
                        // dibuat (lihat getOrCreateChatForBooking/getOrCreateDirectChat),
                        // sementara otherPartyPhotoUrl TIDAK PERNAH diisi di
                        // sana. Akibatnya foto lawan bicara tidak pernah muncul
                        // sama sekali, dan juga tidak pernah ikut update saat
                        // lawan bicara mengganti foto profilnya. Sekarang nama
                        // dan foto selalu disegarkan dari dokumen users/creators
                        // yang sebenarnya (dengan cache di memori supaya tidak
                        // membaca ulang dokumen yang sama berkali-kali).
                        if (otherId.isBlank()) return@forEach
                        val profile = displayNameCache[otherId] ?: run {
                            val creator = db.collection(FirestorePaths.CREATORS).document(otherId).get().await()
                            val creatorName = creator.getString("displayName").orEmpty()
                            val creatorPhoto = creator.getString("photoUrl")
                            val result = if (creatorName.isNotBlank()) creatorName to creatorPhoto else {
                                val user = db.collection(FirestorePaths.USERS).document(otherId).get().await()
                                user.getString("name").orEmpty() to user.getString("photoUrl")
                            }
                            displayNameCache[otherId] = result
                            result
                        }
                        if (profile.first.isNotBlank()) {
                            chatsById[chat.chatId] = chat.copy(otherPartyName = profile.first, otherPartyPhotoUrl = profile.second)
                            publish()
                        }
                    }
                }
