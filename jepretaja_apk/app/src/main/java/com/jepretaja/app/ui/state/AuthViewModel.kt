...
class AuthViewModel @Inject constructor(
    ...
    // Penjaga urutan selesai untuk pembacaan profil yang berjalan paralel.
    //
    // authStateFlow() otomatis terpicu TEPAT saat createUserWithEmailAndPassword
    // berhasil — sebelum dokumen users/{uid} sempat ditulis oleh registerCreator/
    // registerCustomer. Pembacaan profil dari trigger itu sering mendapat role
    // lama/kosong. Sesudahnya, layar registrasi memanggil refreshProfile() secara
    // manual yang seharusnya membaca role yang sudah benar — TAPI tanpa penjaga
    // ini, kalau pembacaan pertama (yang basi) kebetulan baru selesai belakangan
    // (jaringan lambat, retry), hasilnya menimpa balik state yang sudah benar:
    // creator yang baru daftar terlihat kembali jadi customer biasa. Setiap
    // pembacaan diberi nomor urut, dan hasilnya hanya diterapkan kalau tidak ada
    // pembacaan yang lebih baru sudah dimulai — siapa pun yang SELESAI paling
    // akhir yang menang, bukan siapa yang MULAI paling akhir.
    private var profileFetchToken = 0L
            ...
            authRepository.authStateFlow()
                ...
                .collect { user ->
                    if (user == null) {
                        ...
                        val token = ++profileFetchToken
                        val profile = withTimeoutOrNull(8_000) {
                            runCatching { authRepository.getCurrentUserProfile() }.getOrNull()
                        }
                        // Lihat komentar di deklarasi profileFetchToken: kalau
                        // refreshProfile() sudah memulai pembacaan yang lebih baru
                        // sementara pembacaan ini masih berjalan, hasil basi ini
                        // dibuang, bukan dipakai menimpa state yang sudah benar.
                        if (token == profileFetchToken) {
                            _uiState.value = AuthUiState(
    ...
    fun refreshProfileThen(onComplete: () -> Unit) {
        viewModelScope.launch {
            refreshProfile()
            onComplete()
        }
